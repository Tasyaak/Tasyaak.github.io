...
window.onload = function (event) {
    ...
    $("input:not([type=\"checkbox\"]), textarea").on("change", function () {
        localStorage.setItem(this.id, this.value);
    });

    $("input:not([type=\"checkbox\"]), textarea").on("input", function () {
        localStorage.setItem(this.id, this.value);
    });
...
window.onload = function (event) {
    ...
    document.getElementById("forma").addEventListener("submit", function (ev) {
        ...
        slapform.submit({
            ...
            let inps = document.querySelectorAll("input:not([type=\"checkbox\"]), #soob");
            for (let i = 0; i < inps.length; ++i) {
                localStorage.removeItem(inps[i].id);
                inps[i].value = "";
            }
    ...
    $("input:not([type=\"checkbox\"])").on("change", function () {
        localStorage.setItem(this.id, this.value);
    });
    ...
    let inps = document.querySelectorAll("input:not([type=\"checkbox\"]), textarea");
    for (let i = 0; i < inps.length; ++i) {
        let val = localStorage.getItem(inps[i].id);
        if (val != null) {
            inps[i].value = val;
